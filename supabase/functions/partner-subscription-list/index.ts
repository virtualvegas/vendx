// GET — list VendX subscription products available to this partner (outbound).
...
serve(async (req) => {
  ...
  let q = supabase
    .from("store_products")
    .select(
      "id,name,slug,short_description,description,price,subscription_price,subscription_interval,category,images,is_subscription,is_active",
      { count: "exact" },
    )
    .eq("is_active", true)
  ...
  if (interval) q = q.eq("subscription_interval", interval);
