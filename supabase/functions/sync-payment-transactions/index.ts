...
serve(async (req) => {
  ...
  try {
    ...
    // Only Stripe needs API sync — PayPal transactions are logged automatically at capture time
    if (!provider || provider === "stripe") {
      try {
        ...
        logStep("Stripe sync failed", { error: stripeError });
      }
    }

    // Count PayPal transactions from DB (already logged inline at payment time)
    const { count: paypalCount } = await supabaseClient
      .from("synced_transactions")
      .select("id", { count: "exact", head: true })
      .eq("provider", "vendx_pay")
      .gte("transaction_date", startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString());

    // Update PayPal sync status to show it's always current
    await supabaseClient
      .from("transaction_sync_status")
      .update({ 
        sync_status: "completed", 
        last_sync_at: new Date().toISOString(),
        transactions_synced: paypalCount || 0,
        error_message: null,
        updated_at: new Date().toISOString()
      })
      .eq("provider", "paypal");

    const hasSuccess = stripeCount > 0 || !stripeError;
    
    return new Response(JSON.stringify({
      success: hasSuccess,
      stripe_transactions: stripeCount,
      paypal_transactions: paypalCount || 0,
      total: stripeCount + (paypalCount || 0),
      stripe_error: stripeError,
      paypal_error: null,
      paypal_note: "PayPal transactions are logged automatically at payment time — no API sync needed.",
    }), {
