...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS card_banner_url text;
...
BEGIN
  SELECT p.id, p.full_name, p.email, p.phone, p.avatar_url, p.job_title,
         ...
         p.card_accent_color, p.card_public, p.division_ids, p.card_banner_url
  INTO _profile
  ...
  RETURN jsonb_build_object(
    ...
    'card_banner_url', _profile.card_banner_url,
    'roles', _roles,
