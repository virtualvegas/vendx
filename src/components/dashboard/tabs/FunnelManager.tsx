...
import { Plus, Edit, Trash2, Eye, ArrowUp, ArrowDown, Settings, Package, Zap, Layers, BarChart3 } from "lucide-react";
...
export default function FunnelManager() {
  ...
  const openEditProduct = (product: FunnelProduct) => {
                ...
                <TabsContent value="products" className="space-y-3">
                  ...
                  {stepProducts.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {stepProducts.map((fp) => (
                        <div key={fp.id} className="flex items-center justify-between p-2 rounded border">
                          <div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-muted/50 rounded-lg">
              <p className="text-2xl font-bold">{funnelSteps.length}</p>
              <p className="text-sm text-muted-foreground">Steps (Selected)</p>
            </div>
            <div className="text-center p-4 bg-muted/50 rounded-lg">
              <p className="text-2xl font-bold">{stepProducts.length + stepAddons.length}</p>
              <p className="text-sm text-muted-foreground">Items (Selected Step)</p>
            </div>
          </div>
        </CardContent>
      </Card>
