...
import { Percent, Search, RefreshCw, Edit, Plus, Monitor, Building2 } from "lucide-react";
...
const ProfitSplitsManager = () => {
  ...
  const splitMutation = useMutation({
    mutationFn: async ({ machineId, vendxPct }: { machineId: string; vendxPct: number }) => {
      // If editing, end the current split
      if (editingSplit) {
        await supabase
          .from("machine_profit_splits")
        ...
        // Check if there's an existing active split and end it
  ...
  // Merge machines with their splits
  ...
  // Machines without splits for dropdown
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold text-green-500">{stats.configured}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Not Configured</p>
            <p className="text-2xl font-bold text-yellow-500">{stats.unconfigured}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Avg VendX Share</p>
            <p className="text-2xl font-bold text-primary">{stats.avgVendxPct}%</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Machine Profit Splits ({filteredMachines.length})</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {filteredMachines.map((machine: any) => (
                  <TableRow key={machine.id}>
                    ...
                    <TableCell>
                      {machine.split ? (
                        <span className="font-bold text-primary">{machine.split.vendx_percentage}%</span>
                      ) : (
                        <Badge variant="secondary">Default (70%)</Badge>
                      )}
                    </TableCell>
                    <TableCell>
                      {machine.split ? (
                        <span className="font-bold text-green-500">{machine.split.business_owner_percentage}%</span>
                      ) : (
                    ...
                    <TableCell>
                      {machine.split ? (
                        <span className="text-sm">{new Date(machine.split.effective_from).toLocaleDateString()}</span>
                      ) : (
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Set the revenue sharing percentage between VendX and the business owner
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-4">
              ...
              <Slider
                value={[vendxPercentage]}
                onValueChange={(v) => setVendxPercentage(v[0])}
                min={0}
                max={100}
                step={5}
              />
            ...
            <div className="bg-muted/50 p-4 rounded-lg">
              ...
              <div className="flex justify-between">
                <div>
                  <p className="text-xs text-muted-foreground">VendX Global</p>
                  <p className="font-bold text-primary">${((vendxPercentage / 100) * 1000).toFixed(0)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Business Owner</p>
                  <p className="font-bold text-green-500">${(((100 - vendxPercentage) / 100) * 1000).toFixed(0)}</p>
                </div>
              </div>
            </div>
          </div>
